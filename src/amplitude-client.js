...
AmplitudeClient.prototype.saveEvents = function saveEvents() {
  try {
    const unsentEvents = JSON.stringify(this._unsentEvents.map(({event}) => event));

    if (AsyncStorage) {
      AsyncStorage.setItem(this.options.unsentKey + this._storageSuffix, unsentEvents);
    } else {
      this._setInStorage(localStorage, this.options.unsentKey, unsentEvents);
    }
  } catch (e) {}

  try {
    const unsentIdentifys = JSON.stringify(this._unsentIdentifys.map(unsentIdentify => unsentIdentify.event));
